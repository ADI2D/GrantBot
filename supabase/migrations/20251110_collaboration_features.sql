...
-- ============================================================================
-- Real-time presence, inline comments, mentions, notifications, and activity feed
-- ============================================================================
...
CREATE INDEX idx_proposal_presence_proposal ON proposal_presence(proposal_id);
CREATE INDEX idx_proposal_presence_last_seen ON proposal_presence(last_seen_at DESC);
...
CREATE INDEX idx_inline_comments_proposal ON proposal_inline_comments(proposal_id);
CREATE INDEX idx_inline_comments_section ON proposal_inline_comments(section_id);
CREATE INDEX idx_inline_comments_resolved ON proposal_inline_comments(resolved);
CREATE INDEX idx_inline_comments_created ON proposal_inline_comments(created_at DESC);
...
CREATE TABLE IF NOT EXISTS proposal_mentions (
  ...
  mention_text TEXT NOT NULL, -- The actual @mention text
...
CREATE INDEX idx_mentions_mentioned_user ON proposal_mentions(mentioned_user_id);
CREATE INDEX idx_mentions_proposal ON proposal_mentions(proposal_id);
CREATE INDEX idx_mentions_read ON proposal_mentions(read, mentioned_user_id);
...
CREATE INDEX idx_section_revisions_section ON proposal_section_revisions(section_id);
CREATE INDEX idx_section_revisions_proposal ON proposal_section_revisions(proposal_id);
CREATE INDEX idx_section_revisions_created ON proposal_section_revisions(created_at DESC);

-- 5. ENHANCED ACTIVITY LOGS
-- ============================================================================
-- Extend existing activity_logs for richer collaboration tracking
...
-- Add index for section-based filtering
CREATE INDEX IF NOT EXISTS idx_activity_logs_section ON activity_logs(section_id);
...
-- Trigger to auto-save revisions
...
-- 8. COMMENTS
-- ============================================================================
COMMENT ON TABLE proposal_presence IS 'Real-time presence tracking - who is viewing/editing which proposal';
COMMENT ON TABLE proposal_inline_comments IS 'Inline comments on specific text selections within proposal sections';
COMMENT ON TABLE proposal_mentions IS 'User mentions in comments and notifications';
COMMENT ON TABLE notifications IS 'System-wide notifications for users';
COMMENT ON TABLE proposal_section_revisions IS 'Version history for proposal section content';
