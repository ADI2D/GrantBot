...
/**
 * POST /api/opportunities/analyze-document
 *
 * Analyzes an uploaded grant document (PDF, Word) using AI to extract details.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 4. Calculate alignment score based on organization profile
    // 5. Extract compliance requirements
    // 6. Store in database

    // For now, return a mock response
    const mockOpportunity = {
      name: `Grant from ${file.name.replace(/\.[^/.]+$/, "")}`,
